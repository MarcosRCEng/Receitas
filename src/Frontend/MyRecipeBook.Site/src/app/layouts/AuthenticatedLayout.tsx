    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div>
            ...
            <p className="text-sm text-slate-500">Area autenticada</p>
          </div>
          <nav className="flex gap-2 overflow-x-auto pb-1 lg:hidden" aria-label="Navegacao principal">
            {navItems.map((item) => (
              <NavLink
                className={({ isActive }) =>
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[220px_1fr] lg:px-8">
        <aside className="hidden lg:block">
          <nav className="sticky top-28 space-y-1" aria-label="Navegacao principal">
            {navItems.map((item) => (
              <NavLink
                className={({ isActive }) =>
                  `block rounded-md px-3 py-2 text-sm font-semibold transition ${
                    isActive ? 'bg-brand-100 text-brand-700' : 'text-slate-600 hover:bg-white hover:text-slate-950'
                  }`
                }
                key={item.to}
                end={item.end}
